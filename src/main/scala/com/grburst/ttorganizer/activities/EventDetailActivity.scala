...
import com.grburst.ttorganizer.libtt.parser.EventDetailParser
...
class EventDetailActivity extends SActivity with Contexts[Activity] with Styles {
  ...
  def ttrMatchesListable(): Listable[Match, HorizontalLinearLayout] =
    Listable[Match].tr {
  ...
  override def onCreate(savedInstanceState: Bundle) = {
    ...
    setContentView {
      (l[VerticalLinearLayout](
        ...
        w[ListView] <~ ttrMatchesListable.listAdapterTweak(eventDetailParser.get())) <~ padding(left = 4 dp, right = 4 dp)).get
    }
