...
import com.grburst.ttorganizer.libtt.parser.EventsParser
...
class EventsActivity extends SActivity with Contexts[Activity] with Styles {
  ...
  def ttrEventsListable(): Listable[Event, VerticalLinearLayout] =
    Listable[Event].tr {
  ...
  override def onCreate(savedInstanceState: Bundle) = {
    ...
    setContentView {
      (l[VerticalLinearLayout](
        ...
        w[ListView] <~ ttrEventsListable.listAdapterTweak(eventParser.get())) <~ padding(left = 4 dp, right = 4 dp)).get
    }
  }
